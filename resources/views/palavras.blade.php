...
<div class="flex flex-col justify-center items-center gap-2">
    <div class="flex justify-around items-center gap-2">
        ...
        <a href="{{ route('palavranovo') }}" class="bg-emerald-500 rounded-md py-1 px-4 focus:bg-slate-50
        ...
        <tr class="bg-violet-100 even:bg-violet-200">
            ...
            <td class="flex pl-2 pr-1 w-fit py-5 flex-nowrap gap-x-2" scope="col">
                <form method="POST" action="{{ route('palavradelete', ['id'=> $palavra->id]) }}">
                    ...
                    {{ csrf_field()}}
                    <a href="{{ route('palavraform', ['id' => $palavra->id]) }}" class='bg-emerald-500 rounded-full py-1 px-4 focus:bg-slate-50
